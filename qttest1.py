from PyQt5.QtWidgets import  (QApplication , QMainWindow,
                              QLineEdit,QPushButton,QLabel,QVBoxLayout,QWidget)
import sys
from random import randint
class AnotherWindow(QWidget):
    """
        이 윈도우는 QWidet 이고 만약 부모가 없다면 우리가 원하는 대로 떠다니지 않는것처럼 보인다
    """
    def __init__(self):
        super().__init__()
        ...
        self.setLayout(layout) # TheOtherWindow에
...
class TheOtherWindow(QWidget):
    ...
    def update_label(self):
        self.label.setText("another window %d " % randint(70,800))
...
class MainWindow(QMainWindow):
    ...
    def show_new_window(self,checked): #버튼을 누를때 마다 여기 들어온다
